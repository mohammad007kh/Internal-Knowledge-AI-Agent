...
**GraphInterrupt bridge (load-bearing):** the CURRENT clarification path emits via a `GraphInterrupt` exception handler in `chat_stream_service.py` whose value is a plain string passed to `ChatStreamEvent.clarification(question)`. Structured options CANNOT ride a bare string: either (a) make the planner's `needs_clarification` path raise/return a STRUCTURED payload (dict with `question`/`options`/`allow_free_text`) and update the handler to detect dict-vs-string, or (b) emit the clarification event directly from the stream service when the final state carries the clarification payload (no exception). Choose (b) if the agentic graph ends the turn normally with a clarification state field — document the choice. ALSO update the `ChatStreamEvent.clarification()` factory SIGNATURE (currently `question: str` only) to accept the optional options payload.
